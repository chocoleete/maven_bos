...
<script type="text/javascript">
	...
	$(function() {
		...
		window.setTimeout(function(){
			$.messager.show({
				title:"消息提示",
				msg:'欢迎登录，${loginUser.username}！ <a href="javascript:void(0)" onclick="top.showAbout();">联系管理员</a>',
				timeout:5000
			});
		},3000);
		/*************/
...
//			alert("修改密码");
			...
			if(v) {
				...
				if(v1==v2) {
					...
					$.post(url,{password:v0,passwordNew:v1},function(date){
						//判断
						if(date) {
							//如果返回结果为true表示修改成功，false表示修改失败
    ...
    <div id="editPwdWindow" class="easyui-window" title="修改密码" collapsible="false" minimizable="false" modal="true" closed="true" resizable="false"
        ...
        <div class="easyui-layout" fit="true">
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
                <form id="editForm">
					<table cellpadding=3>
						<%--添加一个原密码的输入框 为修改密码窗口中的输入框添加校验功能--%>
						<tr>
							<td>原密码：</td>
							<td><input id="txtOldPass" type="Password" data-options="required:true,validType:'length[3,16]'"
									   class="txt01 easyui-validatebox" /></td>
						</tr>
						<tr>
							<td>新密码：</td>
							<td><input id="txtNewPass" type="Password" data-options="required:true,validType:'length[3,16]'"
									   class="txt01 easyui-validatebox" /></td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td><input id="txtRePass" type="Password" data-options="required:true,validType:'length[3,16]'"
									   class="txt01 easyui-validatebox" /></td>
						</tr>
					</table>
				</form>
