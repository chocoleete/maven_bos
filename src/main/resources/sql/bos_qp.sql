...
alter table qp_noticebill add constraint FK_Reference_20 foreign key (user_id)
      references t_user (id) on delete restrict on update restrict;

alter table qp_noticebill add constraint FK_Reference_30 foreign key (staff_id)
      references bc_staff (id) on delete restrict on update restrict;

alter table qp_workbill add constraint FK_Reference_40 foreign key (staff_id)
      references bc_staff (id) on delete restrict on update restrict;

alter table qp_workbill add constraint FK_workbill_noticebill_fk_50 foreign key (noticebill_id)
